package com.ragas.springboot.weatherapp.web;
...
import com.ragas.springboot.weatherapp.controller.WeatherApiController;
import com.ragas.springboot.weatherapp.integration.ows.Weather;
import com.ragas.springboot.weatherapp.integration.ows.WeatherEntry;
...
public class WeatherApiControllerTest {
	...
	public void weather() throws Exception {
		...
		given(mockService.getWeather("uk", "london")).willReturn(weather);
		this.mvc.perform(get("/api/weather/now/uk/london")).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.weatherId", is(800))).andExpect(jsonPath("$.weatherIcon", is("01d")))
				.andExpect(jsonPath("$.timestamp", is("1970-01-01T00:20:34Z")));
		verify(mockService).getWeather("uk", "london");
	}
	...
	public void weatherForecast() throws Exception {
		...
		given(mockService.getWeatherForecast("be", "brussels")).willReturn(forecast);
		this.mvc.perform(get("/api/weather/weekly/be/brussels")).andExpect(status().isOk())
		...
		verify(mockService).getWeatherForecast("be", "brussels");
	}

	private static WeatherEntry createWeatherEntry(double temperature, int id, String icon, Instant timestamp) {
